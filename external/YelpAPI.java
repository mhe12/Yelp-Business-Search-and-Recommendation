...
public class YelpAPI {
	private static final String API_HOST = "https://api.yelp.com";//拿餐馆信息
	private static final String DEFAULT_TERM = "dinner";
	private static final int SEARCH_LIMIT = 20;
	private static final String SEARCH_PATH = "/v3/businesses/search";

	private static final String TOKEN_HOST = "https://api.yelp.com/oauth2/token";//网站地址
	private static final String CLIENT_ID = "hn0hKAYqGXMdjQaz5epMiw";
	private static final String CLIENT_SECRET = "9ppr1PW71DGEqx9GklItHTftxKo0FSCFD2ZjQgWBGb7Z9MPhtCVUiI0hwGoaSc98";
	private static final String GRANT_TYPE = "client_credentials";
	private static final String TOKEN_TYPE = "Bearer";

	// Cache your access token so you get faster access to our data by
	// eliminating an extra request for each API call.
	private static String accessToken;//拿回来的token
	 ...
	 */
	// getAccessToken得到临时密码
	private String getAccessToken() {
		if (accessToken != null) {
			return accessToken;
		}

		try {// 发送HTTP到外界的server,new一个URL用TOKEN_HOST，打开一个HTTP
				// connction:openConnection()
			HttpURLConnection connection = (HttpURLConnection) new URL(TOKEN_HOST).openConnection();

			// set HTTP POST method
			connection.setDoOutput(true);
			connection.setRequestMethod("POST");// yelp要我们用"POST"
			...
			// get response 看是不是200 ok 404 挂了
	 ...
	 */
	// search is for searching the near restaurants
	 ...
	 */
	// queryAPI打印输出不需要的field就不在了比如distance
